...
public class CommitteeMemberService {
    ...
    public long countAll() {
        return committeeMemberMapper.count(null, null);
    }
    ...
    @Transactional(readOnly = true)
    public PageResult<CommitteeMember> page(String q, Boolean isActive, Integer page, Integer size) {
        String query = Paging.normalizeQuery(q);
        int p = Paging.normalizePage(page);
        int s = Paging.normalizeSize(size, 10, 100);

        long total = committeeMemberMapper.count(query, isActive);
        ...
        List<CommitteeMember> items = committeeMemberMapper.findPage(query, isActive, offset, s);
